...
# bnn0414 path resolution
_CODE_DIR = _SCRIPT_DIR                              # experiments_0404/
...
_PRESETS = {
    "main": {
        # BNN 主线：bnn-baseline + bnn-data-mono
        # 全部从头训练，不复用旧 checkpoint
        "models": ["bnn-baseline", "bnn-data-mono"],
        "modules": {
            "train":               True,
            "eval_fixed":          True,
            "eval_repeat":         False,
    ...
    "appendix": {
        "models": ["bnn-phy-mono", "bnn-data-mono-ineq"],
        "modules": {
            "train":               True,
            "eval_fixed":          True,
            "eval_repeat":         False,
...
def main():
    ...
    logger.info("-" * 65)
    logger.info("  BNN 模型说明：所有 BNN 模型均从头训练，")
    logger.info("  不复用旧 checkpoint。使用 BayesianMLP 替代 HeteroMLP。")
    logger.info("=" * 65)
    ...
    if modules.get("physics_consistency"):
        logger.info("\n[PHASE] 物理先验一致性分析")
        # 对所有已选模型分别运行（特别是 bnn-phy-mono 系列）
        phy_models = [m for m in models if m in ("bnn-phy-mono", "bnn-data-mono-ineq")]
        # 始终也对主文模型跑一次作为基线对比
...
║  默认全跑主线：                                              ║
║    python run_0404.py                                        ║
║    （RUN_CONFIG["preset"] = "main" 时跑                      ║
║     bnn-baseline + bnn-data-mono）                           ║
║                                                              ║
║  跑附录模型：                                                ║
...
║    RUN_CONFIG["custom_models"] = ["bnn-phy-mono"]            ║
