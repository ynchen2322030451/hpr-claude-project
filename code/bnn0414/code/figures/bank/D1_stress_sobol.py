"""D1 — First-order Sobol indices for stress (main Fig 4 left).

Horizontal bar chart with 90% CI whiskers. Category-coloured bars:
  - Elastic/structural (teal): E_slope, E_intercept, nu
  - Thermal expansion (orange): alpha_base, alpha_slope
  - Conductivity (purple): SS316_T_ref, SS316_k_ref, SS316_alpha
Bars whose CI spans zero are greyed out regardless of category.
"""
...
def draw(fig=None, ax=None):
    ...
    ax.set_title("Coupled steady-state max stress",
                 fontsize=FS["title"], pad=4)
    ax.invert_yaxis()
...
def main():
    ...
    fig, ax = draw()
    panel_label(ax, "(A)")
    written = savefig(fig, "D1_stress_sobol", _OUT_DIR)
    for w in written:
        print("wrote", w)
