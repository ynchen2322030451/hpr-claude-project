"""B1 — Stress parity plot (primary predictive-behaviour panel).

Reference-matched design:
  - Hexbin density scatter (turbo colormap) with colorbar
  - Red linear fit line
  - Light-blue 90% predictive interval band
...
def draw(fig=None, ax=None, show_colorbar=True):
    ...
    yt = y_true[:, STRESS_IDX]
    yp = mu[:, STRESS_IDX]
    ys = sigma[:, STRESS_IDX]

    row = metr[metr.output == STRESS_COL].iloc[0]
    r2   = row["R2"]
    rmse = row["RMSE"]
    picp = row["PICP"]
    ...
    # --- 90% PI band (sorted by true, smoothed) ---
    order = np.argsort(yt)
    yt_s = yt[order]
    ...
    ax.set_ylabel("Predicted mean stress (MPa)")
...
def main():
    ...
    fig, ax = draw()
    panel_label(ax, "(A)")
    written = savefig(fig, "B1_stress_parity", _OUT_DIR)
    for w in written:
        print("wrote", w)
