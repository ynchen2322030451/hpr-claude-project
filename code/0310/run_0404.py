...
def main():
    ...
    if modules.get("figures_main") or modules.get("figures_appendix"):
        ...
        env = {
            "MODELS":          ",".join(models),
            "FIGURES_MAIN":    "1" if modules.get("figures_main")     else "0",
            "FIGURES_APPENDIX":"1" if modules.get("figures_appendix") else "0",
        }
        ok = run_script("run_figures_0404.py", env, logger, dry_run)
        results["figures"] = ok
