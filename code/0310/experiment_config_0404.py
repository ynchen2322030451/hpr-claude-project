...
# 新实验根目录（本次重构产出统一写这里）
EXPR_ROOT_0404 = os.path.join(_SCRIPT_DIR, "experiments_0404")

# 旧实验目录（只读参考，不覆盖）
EXPR_ROOT_OLD  = os.path.join(_SCRIPT_DIR, "experiments_phys_levels")

# 共享 fixed split（直接复用旧 fixed split）
FIXED_SPLIT_DIR = os.path.join(EXPR_ROOT_OLD, "fixed_split")

# 数据路径（服务器路径，本地不可达时回退到 fixed_split CSV）
...
# 本地调试时可指向任意可读 CSV（留空则自动拼接 fixed_split）
CSV_PATH_LOCAL    = ""
...
# k·σ 扰动倍数。主文报告 k=1.0（标准设计裕量）；k 序列用于风险-扰动曲线。
# 3σ 因绝大多数样本超阈而失去区分度，已移出主文范围（可保留附录）。
RISK_PROP_SIGMA_K        = [0.5, 1.0, 1.5, 2.0]   # 主文用，去掉 3σ
RISK_PROP_SIGMA_K_MAIN   = 1.0                     # 主文单点报告的标准扰动倍数
...
INVERSE_CALIB_PARAMS   = [    # 标定参数子集（应力主控E_intercept + keff主控alpha_base/alpha_slope + 应力第三敏感因子SS316_k_ref）
