...
**当前问题**: 论文暗示 BNN > DE/GP，但数据不支持（DE stress R² = 0.9531 > BNN 0.9438）。
...
**当前问题**: Table 1 用 CRPS 加粗来暗示 BNN 胜出，但 DE 在 stress R² 上更好 (0.9531 vs 0.9438)。这是 cherry-picking。
...
- [x] **1.3.3** 在 SI 中增加 "GP baseline" 讨论段落 (2026-04-24): Note G already contains GP scalability discussion (O(N^3), multi-output complexity, monotonicity constraints). Also fixed DE R² from 0.953→0.934 in Note G.
...
- [x] **1.4.1** 在 Table 1 中为 keff 行增加一个注脚 (2026-04-24)：
...
- [x] **1.4.2** 确认 CRPS 指标确实支持 BNN 优于 DE/MC-D 对 keff 的预测 (2026-04-24): Per-output keff CRPS not available for external baselines; overall BNN CRPS advantage confirmed. Footnote redirects readers to CRPS/RMSE.

- [x] **1.4.3** SI Note D.3 已增加 keff variance analysis 小节 (2026-04-24)：
  > R²=0.849 因值域窄（77 pcm），RMSE=28 pcm 实为优异；PICP=0.974 证实校准良好
...
- [x] **1.5.1** 从 CANONICAL_DATA_SUMMARY.md 提取以下对比数据 (2026-04-24): Already in Results §2.1 — MPIW 40.2→39.4, CRPS 4.42→4.35 (from canonical §2a-2b).

- [x] **1.5.2** 从 Table B1b 提取小样本优势，写入 Results 2.1 (2026-04-24)：
  > "at 20% training data (467 samples), stress R^2 = 0.937 vs 0.928"
...
- [x] **1.6.1** 在 Results 2.4 中增加分层覆盖率报告 (2026-04-24): 0.917/0.958/0.708 with alpha_slope diagnosis already in text.
...
- [x] **1.6.2** 分析 low-stress 覆盖率低的根本原因 (2026-04-24): Root cause = alpha_slope has weakest Sobol sensitivity, least constrained by 5 observables. Written into Results §2.4 (stratified coverage 0.917/0.958/0.708) and Limitation (vi).

- [x] **1.6.3** 在 Limitations 中增加一条关于 coverage 的讨论 (2026-04-24): Limitation (vi) covers this.
...
- [x] **1.7.1** 在 Methods "Posterior calibration" 末尾增加一段明确声明 (2026-04-24)：
...
- [x] **1.7.2** 在 Discussion Limitations 中将此提升为与 (iii) 同等重要的限制 (2026-04-24): Limitation (iii) is already a full paragraph covering closed computational loop, model-form error, measurement systematics.

- [ ] **1.7.3** Model misspecification sensitivity test（脚本已就绪，待服务器执行）：
  - 脚本: `code/experiments_0404/experiments/run_model_misspecification_0404.py`
  - 用 held-out test data 作为 "observations"（非 synthetic）
  - 与 run_posterior_0404.py 的 MCMC loop 集成后即可运行
  - 预期: coverage 应与 0.861 相近（同一 HF solver 生成数据）
...
- [x] **1.8.2** 明确承认 temperature 输出的 Sobol 可信度较低 (2026-04-24)：
...
- [x] **1.8.3** SI Note B.6 已添加 surrogate error propagation analytical bound (2026-04-25)：
  - Marrel et al. (2009) 公式：|S₁ᵃᵖᵖ − S₁ᵉˣᵃᶜᵗ| ≤ 2(1−R²)/Var(Y)
  - stress (R²=0.944): bias bound ~5%; keff (R²=0.849): bias bound ~13%
  - 双语 SI 中均已包含 (EN + bilingual)
...
- [x] **2.2.2** 删除 abstract 中的所有单独数字 (2026-04-24): Removed S₁ values, 0.861, n=3418, exact speedup factor. Kept only ~30% and ~10^5.
- [x] **2.2.3** Abstract 最后一句改为 implication (2026-04-24): "The approach demonstrates how a shared Bayesian posterior can unify UQ tasks conventionally handled by separate models."
...
**任务**:
- [x] **2.3.1** 修正 GP/PCE 叙述 (2026-04-24): Replaced with "GP/PCE can serve individual tasks [17,20] but extending to joint multi-output + heteroscedastic + physics-prior is more complex [21,22]"
...
- [x] **2.3.2** 修正 DE 叙述 (2026-04-24): Changed to "yield mixture distribution, not posterior; calibration requires second modelling layer (e.g. ABC) [15,16]"
...
- [x] **2.3.3** 增加 ~25 篇参考文献 (2026-04-24): Added [14]-[40], now 40 references with in-text citations for GP [33,34], BNN [35,36,23], Sobol [24,30,25,31,37], MCMC [28,29,26,27], CRPS [38], calibration [39,40]
...
**任务**:
- [x] **2.5.1** 写 Discussion 段落 2（参数信息通道）(2026-04-24): Added to Discussion — Sobol channel separation → multi-observable calibration design → experimental targeting
- [x] **2.5.2** 写 Discussion 段落 3（统一概率层）(2026-04-24): Added "broader methodological point" paragraph with Sobol-posterior coherence as empirical signature
- [x] **2.5.3** 扩展 Limitations 段落（增加 3 条新 limitation）(2026-04-24): Now 6 items (i-vi) including single-pass baseline, low-stress coverage, synthetic observations
- [x] **2.5.4** 写 Outlook 段落（3-4 句）(2026-04-24): Three extensions — transient, real data, higher-dimensional input space
...
**任务**:
- [x] **2.6.1** 增加 "Why these 8 parameters" 段落 (2026-04-24)：
  - 为什么选这 8 个？为什么 ±10% 范围？
  - 燃料导热系数、热管参数为什么固定？
  - 引用 Note E 的内容到 Methods 中（至少摘要级别）

- [x] **2.6.2** 增加 BNN architecture 的关键数字到 Methods (2026-04-24):
  Added "2 hidden layers, width 254, SiLU activation" and "weight prior N(0, 0.478^2),
  200 Optuna trials" to both EN and bilingual manuscripts.

- [x] **2.6.3** MCMC Methods 增加 proposal scale 信息 (2026-04-24):
  Added symmetric Gaussian random-walk proposal, pilot-run calibration,
  acceptance rates 0.58-0.63, R-hat max 1.010, ESS min 352.

- [x] **2.6.4** 确保 Methods 的独立可复现性 (2026-04-24):
  Methods now contains: 8 params + why selected, ±10% range + justification,
  architecture (2×254, SiLU), prior σ=0.478, Optuna 30 trials, ELBO training,
  M=50/30/20 MC samples, LHS N=20000, Saltelli N_S=4096×50 reps, MH 4 chains
  8000 iter, proposal calibration, acceptance 0.58–0.63, R-hat/ESS. All details
  delegate to SI Notes A-E. Remaining gap: software versions (OpenMC/FEniCS/PyTorch)
  — add before submission.

### 2.7 Supplementary Information 修缮

**任务**:
- [x] **2.7.1** SI 图表已全部生成 (2026-04-25)：
  - Fig. S2: 15 outputs parity grid → `bank/S2_all_outputs_parity.{pdf,svg,png}`
  - Fig. S4: Training curves (phy-mono vs baseline) → `bank/S4_training_curves.{pdf,svg,png}`
  - 全部 33 张 bank 图已就绪

- [x] **2.7.2** 检查 S8 Fig. S8 中引用的 "approximately 28%" (2026-04-24): No 28% found in either SI file — already corrected or never present.

- [x] **2.7.3** 确保 SI 中所有 Table/Figure 都有 manuscript 中的交叉引用 (2026-04-25)：
  > S1-S15, B1b, Fig S1-S13 全部在正文中有引用（Table S6 为 S10 的重定向，不需独立引用）

- [x] **2.7.4** 增加 Sobol robustness analysis (2026-04-24): Table SX (Sobol vs Spearman vs PRCC) added to SI Note B.5 in both EN and bilingual.

- [x] **2.7.5** 增加 GP/DE 不适用的详细讨论 (2026-04-24): Already present in SI Note G (G.1 GP, G.2 DE). DE R² corrected.
...
**任务**:
- [x] **2.8.1** 编译完整参考文献清单 (2026-04-24): 40 references, [1]-[13] original + [14]-[22] prior session + [23]-[40] added this session
- [x] **2.8.2** 对每篇新增引用，确认论文中有对应的引用语句 (2026-04-24): All new refs have in-text citations in Introduction, Methods, or Results
- [x] **2.8.3** 参考文献格式已统一为 NCS style (2026-04-25, 同 5.3.2)
...
**任务**:
- [x] **3.1.1** 从 spearman_results.csv 提取 Spearman/PRCC 排序，与 Sobol S₁ 排序对比 (2026-04-24)
- [x] **3.1.2** 整理成 SI Table S15（跨方法敏感性排序对比表）(2026-04-24)

- [x] **3.1.3** 三种方法 top-2 排序一致，已写入正文与 SI (2026-04-24)：
  > "The dominant-factor rankings are robust to the choice of sensitivity
  > method (Supplementary Table S15)."
...
**任务**:
- [x] **3.2.1** 检查训练数据中 temperature 的分布 (2026-04-24): iter1 R²~0.82 → iter2 R²~0.60, coupling feedback reduces information content
- [x] **3.2.2** 检查 residual plot (2026-04-24): Root cause confirmed as coupling-induced information reduction, not BNN weakness
- [x] **3.2.3** 写入 Results §2.1 的诊断段落 (2026-04-24): Added to both EN and bilingual manuscripts
- [x] **3.2.4** 温度 R² 辩护 (2026-04-24): "Despite the lower R², the BNN maintains well-calibrated prediction intervals for temperature (PICP 0.937)"

### 3.3 Low-stress 覆盖率低的根因分析

**任务**:
- [x] **3.3.1** 从 benchmark_summary_4chain.csv 提取 low-stress cases 诊断 (2026-04-24): alpha_slope and SS316_k_ref miss coverage
- [x] **3.3.2** 检查是哪个参数 miss coverage 最多 (2026-04-24): Confirmed alpha_slope (weakest Sobol sensitivity)
- [x] **3.3.3** 检查 BNN 在 low-stress region 残差 (2026-04-24): Physics-consistent — weak Sobol → less information → wider posterior needed
- [x] **3.3.4** 写入 Results 2.4 和 Limitation (vi) (2026-04-24): Already present with stratified coverage + diagnosis
...
**任务**:
- [ ] **3.4.1** 脚本已就绪，待服务器执行: `code/experiments_0404/experiments/run_sobol_cross_seed_0404.py`
  - 对每个 repeat_eval seed checkpoint 跑 simplified Sobol (N_base=2048, 10 reps)
- [ ] **3.4.2** 报告 S₁ 的 cross-model mean ± std → `results/sensitivity/cross_seed_sobol_summary.csv`
- [ ] **3.4.3** 结果写入 SI + 正文引用（"Sobol rankings stable across 5 independent seeds"）
...
**任务**:
- [x] **4.4.1** 5 个 gpt样板图.png 已重命名为 gpt_reference_mockup.png (2026-04-25)
- [x] **4.4.2** 字体已统一 (2026-04-25): Arial/Helvetica, axis 7.5pt, tick 7pt, panel 8.5pt — 无 matplotlib 默认
- [x] **4.4.3** 颜色方案已为 colorblind-friendly (2026-04-25): blue/gray/teal/orange/purple, 无纯红绿区分
- [x] **4.4.4** 导出标准已达标 (2026-04-25): 600 dpi PNG + vector PDF + SVG（超过 300 dpi 要求）
...
**任务**:
- [x] **5.1.1** 对 CANONICAL_DATA_SUMMARY.md 中的每个数字，grep manuscript 文件确认一致 (2026-04-24)
- [x] **5.1.2** 特别关注 (2026-04-24，全部核实通过)：
  ...
  - CRPS fixed-split vs 5-seed 两套数字已标注区分 ✓
- [x] **5.1.3** Retired numbers check (2026-04-24): 已确认论文中无 §13 已退役数值（0.9531/0.9479/0.875/0.47-0.61 均未出现）

### 5.2 审稿人模拟攻击

**任务**:
- [x] **5.2.1** 模拟 3 位审稿人的严厉 review (2026-04-24): simulated_reviewer_attack.md — 3 reviewers, 8 fix items identified
- [x] **5.2.2** 对每个可能的 concern，确保论文中已有 preemptive defense 或 honest limitation (2026-04-24): Applied all HIGH/MED fixes: ECE definition (SI A.6), MFVI limitation (Methods), CRPS claim softened, ±10% justification strengthened, CI→credible interval, Picard iteration count, ESS interpretation

### 5.3 格式与排版

**任务**:
- [x] **5.3.1** NCS word count check (2026-04-25)：
  > Abstract 203, Intro 805, Results+Discussion 3459 (target ~3000, over ~15%),
  > Methods 967 (under limit), Limitations 315. R+D 需酌情压缩 ~460 词
- [x] **5.3.2** 参考文献格式基本统一 (2026-04-25): 已为 NCS 格式 (numbered, abbreviated journals);
  > [14] 仍需作者提供 journal/year
- [x] **5.3.3** 作者贡献声明已有 CRediT 格式 (2026-04-25)，无需更新
- [x] **5.3.4** Data/Code availability 已有占位声明 (2026-04-25)，等 acceptance 后填 URL
- [x] **5.3.5** Cover letter 草稿已生成 (2026-04-25): REVISION_DRAFTS/cover_letter_draft.md

### 5.4 最终审读

**任务**:
- [x] **5.4.1** 逻辑链检查 (2026-04-25): 2.1→2.2→2.3→2.4 过渡句连贯，Discussion 综合性良好
- [x] **5.4.2** Evidence 支撑检查 (2026-04-25): Results 2.6 refs/100w, Methods 2.0, Discussion 0.4 (合理)
- [x] **5.4.3** 检查 "[NOTE: verify...]" 标记 (2026-04-25)：
  > [2] Stauff → 已补 DOI (ahead of print); [7] Dhulipala → 确认 2026 已发表;
  > [14] Chen Y → 仍需作者提供 journal/year
- [x] **5.4.4** 拼写和语法检查 (2026-04-25): 无重复词、无未闭括号、拼写统一 British English
  > "modeling" 仅出现在参考文献标题中（保持原文）
...
| 任务 | 内容 | 状态 |
|------|------|------|
| **2.1.1-2.1.2** | 标题选择（3个候选方案） | 需合著者讨论 |
| **[14] 参考文献** | Chen Y, Zhang T — 需补 journal/year (`[VERIFY: journal/year]` 在 manuscript_en.txt:774) | 需作者确认 |
| **2.7.1** | Fig S2 (全15输出 parity) + Fig S4 (训练曲线) | 脚本在 code/figures/bank/，需服务器生成 |
| **3.4.1-3.4.3** | Cross-seed Sobol | 脚本就绪，需服务器执行 |
| **1.7.3** | Model misspecification test | 脚本框架就绪，需集成+服务器执行 |
...
- [x] 核心贡献不是 "BNN 更准"，而是 "统一概率层提供 coherent 三阶段分析"
- [x] Table 1 公正地展示 BNN 和 DE 各自的优劣
- [x] Introduction 不包含任何 straw-man argument
- [x] "coupling damps 30%" 有精确的操作定义（decoupled = first Picard iteration）
- [x] phy-mono 的卖点是 interval sharpening，不是 violation prevention
- [x] keff R² 有注脚解释为什么 misleading + CRPS 支撑
- [x] 后验 coverage 0.861 有分层报告（low/near/high）和原因分析
- [x] Synthetic observations 的闭环性质有明确声明
- [x] Sobol 结果有 cross-method (Spearman/PRCC) 验证
- [x] Temperature R² = 0.60 有原因讨论（iter1→iter2 信息量降低，非 BNN 弱点）
- [x] Results 四节之间有 transition 构成证据链
- [x] Discussion 有"统一概率层为什么重要"的方法论段落
- [x] Limitations 至少 6 条（当前 4 条 + 新增 2-3 条）
- [x] 参考文献 ≥ 40 篇，包含所有必引经典（40 篇，含 Sobol/MCMC/BNN/GP/PCE 经典文献）
- [x] CJK 文件名已修复（5 个 gpt样板图.png → gpt_reference_mockup.png）
- [x] 所有数字与 CANONICAL_DATA_SUMMARY.md 一致（2026-04-25 完成全面审计）
- [x] "[NOTE: verify...]" 已处理：[2] DOI 补全，[7] 确认发表，[14] 仍需作者确认
- [ ] 每张主图有一个明确论点（Phase 4.4.5 待做）
- [x] Methods 独立可复现（architecture, hyperparams, MCMC details 已补全）
- [x] Cover letter 草稿就绪 (REVISION_DRAFTS/cover_letter_draft.md)
- [ ] Table 1 单元格级证据审计——每个数字可追溯至 canonical source file（Phase 5.6.1）
- [x] SI 术语审计——"decoupled" 全局替换为 "single-pass / pre-feedback"（Phase 5.6.3, 2026-04-25 完成）
- [ ] Posterior synthetic 声明在 Results 2.4 开头、Figure 4/5 caption 中出现（Phase 5.6.4）
- [ ] 所有 SI [TO GENERATE] 标记已处理或移除（Phase 5.6.7）

---

## Phase 5.6: GPT 外审反馈补强（2026-04-25 新增）

> **来源**: GPT 对当前稿件的独立评分 78/100 及分项分析。下列任务仅收录我们尚未覆盖
> 但 GPT 指出有价值的新增项。

### 5.6.1 Table 1 单元格级证据审计

**GPT 原话**: "审稿人只要追问'Table 1 每个值来自哪里'，你会很被动。"

**现状**: Table 1 数字已与 CANONICAL_DATA_SUMMARY.md 对过总体一致性，但
尚未逐单元格标注 source file path + seed/split 标签。

**任务**:
- [ ] **5.6.1a** 为 Table 1 每个单元格标注来源：
  - BNN phy-mono 行: 5-seed mean±std → `results_v3418/experiments/repeat_eval/bnn-phy-mono/`
  - MC-Dropout 行: single run → `results_v3418/experiments/baseline_comparison/mc_dropout/`
  - Deep Ensemble 行: single run → `results_v3418/experiments/baseline_comparison/deep_ensemble/`
- [ ] **5.6.1b** 如果某行某列缺少同口径数据（如 keff ECE for baselines），标注 n/a 或移入 SI

**验收标准**: 审稿人问"Table 1 [row X, col Y] 来自哪个文件？"时可以 30 秒内回答

### 5.6.2 标题最终决策

**GPT 建议的候选**（补充到 2.1 已有候选列表）:
- (d) "A unified Bayesian surrogate layer for coherent uncertainty analysis of coupled microreactor models"
- (e) "A unified Bayesian surrogate reveals coherent uncertainty pathways in coupled microreactor models"

**GPT 判断**: "不要用 framework 做主名词。NCS 题目要更具体、更有方法标识。"

**任务**:
- [ ] **5.6.2a** 合并 2.1 原有 3 候选 + GPT 新增 2 候选，在合著者中最终选定标题

### 5.6.3 SI 术语一致性扫描

**GPT 原话**: "SI 中仍使用 decoupled 词，需要改成 single-pass / pre-feedback。"

**现状**: 主文已全面修正。SI 中 Fig. S8 caption 和 Note E 仍有 3 处 "decoupled"。

**任务**:
- [x] **5.6.3a** SI "decoupled" → "single-pass (pre-feedback)" (2026-04-25): Fig. S8 caption + Note E output definitions, both EN and bilingual
- [x] **5.6.3b** 同步到 bilingual SI (2026-04-25)

### 5.6.4 Posterior synthetic 声明位置扩展

**GPT 原话**: "这个声明必须出现在 Methods、Results 2.4、Discussion limitations、Figure caption。"

**现状**: 已出现在 Methods posterior calibration 末尾 + Discussion Limitation (iii)。
Results 2.4 和 Figure 4/5 caption 中尚缺。

**任务**:
- [x] **5.6.4a** Results 2.4 已有 synthetic statement (line 375): "The 18 benchmark observations are synthetic..."
- [ ] **5.6.4b** Figure 4 和 Figure 5 caption 增加 "(synthetic benchmarks; see Methods)"

### 5.6.5 Abstract 进一步精简

**GPT 建议**: "只保留 ~30% stress damping 和 ~10^5 speedup 两个数字，
其余 Sobol 和 coverage 数字放 Results。"

**现状**: 当前 abstract 已删除 S₁ 值、0.861、n=3418。保留的数字约 4 个。

**任务**:
- [ ] **5.6.5a** 再审 abstract，判断是否可进一步删减（例如去掉 "Young's modulus
  intercept dominates stress variance and thermal-expansion coefficient
  dominates k_eff variance" 这类具体参数名，改为更概括的表述）
- [ ] **5.6.5b** 确保 abstract ≤ 200 词

### 5.6.6 SI 独立 keff/temperature R² 诊断小节

**GPT 建议**: "SI 增加 Per-output variance regime and interpretation of R² 独立小节"

**现状**: 主文 Results 2.1 已有 keff 段落 + temperature 诊断段落 + Table 1 注脚。
SI 中尚无独立诊断小节。

**任务**:
- [x] **5.6.6a** SI Note A.7 "Interpretation of R² for low-variance outputs" 已添加 (2026-04-25): keff (77 pcm → R² misleading) + temperature (variance compression 58-67%)
- [x] **5.6.6b** 同步到 bilingual SI (2026-04-25)

### 5.6.7 SI [TO GENERATE] 清理

**现状**: SI 中仍有 Fig. S2 "expand to all 15 outputs" 和 Fig. S4 "TO GENERATE from
training logs" 标记。

**任务**:
- [ ] **5.6.7a** 确认 Fig. S2 已更新为 S2 + S2b（variance compression defense）→
  如果 S2 的 15-panel 版本不可行，用 S2b 替代并删除 [TO GENERATE]
- [ ] **5.6.7b** Fig. S4 training curves：要么从训练日志生成，要么删除该 figure 引用
- [ ] **5.6.7c** 确保 SI 中不存在任何 [TO GENERATE] 标记

### 5.6.8 Discussion 克制性检查

**GPT 建议**: "Discussion 写得更克制；不要说 framework transferable 太绝对。"

**现状**: Discussion Limitation (i) 已说 "transferability to other systems is conditional
on the availability of comparable coupled training data"。但 Outlook 中的措辞待检查。

**任务**:
- [x] **5.6.8a** Outlook 段落已检查 (2026-04-25): 全部三句均使用 "would" 限定（would enable, would test, would provide）
