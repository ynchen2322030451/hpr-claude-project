"""
run_figures_0404.py  —  BNN 0414 主文 & 附录图生成

基于 0411 版本改写。主要差异：
    - 模型 id 改为 BNN 4-model 命名（bnn-baseline / bnn-data-mono /
      bnn-phy-mono / bnn-data-mono-ineq）
    - _load_test_preds 读取 BNN 版 test_predictions_fixed.json
      （键：mu / sigma / y_true / epistemic_var / aleatoric_var）
    - Sobol / posterior 的 model_id 过滤更新为 BNN 名
    - HeteroMLP → BayesianMLP 文字更新（fig A7 architecture schematic）

本脚本不触发训练或评估，仅读取已生成的 CSV/JSON 绘图。
若 BNN 结果文件不存在，会退回到 0411/0310 旧 canonical 结果以便渲染，
这种退回仅用于开发期占位，不应在正式产出中使用。

用法（环境变量）：
    python run_figures_0404.py                   # 生成全部图
    FIG_SET=main   python run_figures_0404.py    # 只主文图
    FIG_SET=appendix python run_figures_0404.py  # 只附录图
    FIG_LIST=fig2,fig4 python run_figures_0404.py
...
_EXPR_0404  = os.path.dirname(_HERE)                              # experiments_0404/
_BNN_CODE   = os.path.dirname(_EXPR_0404)                         # bnn0414/code/
_BNN_ROOT   = os.path.dirname(_BNN_CODE)                          # bnn0414/
_CODE_TOP   = os.path.dirname(_BNN_ROOT)                          # code/
_CODE_0310  = os.path.join(_CODE_TOP, "0310")                     # code/0310/
_PROJECT    = os.path.dirname(_CODE_TOP)                          # project root

for _p in [
    os.path.join(_EXPR_0404, "config"),
    _BNN_CODE,
    _CODE_0310,
]:
...
FIG_DIR = resolve_output_dir(
    os.path.join(_EXPR_0404, "figures"),
    script_name=os.path.basename(__file__),
)

# ── 旧数据来源（canonical fallback） ──────────────────────────────────────
_OLD           = EXPR_ROOT_OLD
_OLD_SUR2      = os.path.join(_OLD, "fixed_surrogate_fixed_level2")   # physics-reg (legacy)
_OLD_SUR0      = os.path.join(_OLD, "fixed_surrogate_fixed_base")     # baseline   (legacy)
...
MODEL_LABELS = {
    ...
    "bnn-data-mono":       "BNN + data-mono (constraint-regularized)",
    "bnn-phy-mono":        "BNN + phy-mono",
    "bnn-data-mono-ineq":  "BNN full-constraint surrogate",
}

# 论文 figure 中常用的 "baseline" vs "regularized" 成对比较
# 统一用 bnn-baseline vs bnn-data-mono
BASELINE_ID = "bnn-baseline"
REG_ID      = "bnn-data-mono"
...
def _load_test_preds(model_id):
    """Load test predictions for a BNN model.

    Primary source: BNN test_predictions_fixed.json in model_fixed_eval_dir
      keys: output_cols, n_test, n_mc, mu, sigma, epistemic_var, aleatoric_var, y_true
    Legacy fallback (for development only): 0411 test_predictions_<tag>.json
      keys: mu_te, sigma_te, y_te_true

    Returns (mu, sigma, y_true) arrays of shape (N, 15).
    """
    eval_dir = model_fixed_eval_dir(model_id)
    p_bnn = os.path.join(eval_dir, "test_predictions_fixed.json")
    if os.path.exists(p_bnn):
        with open(p_bnn) as f:
            d = json.load(f)
        mu    = np.array(d["mu"])
    ...
    # Legacy fallback (0411 / 0310)
    legacy_tag = {
        "bnn-baseline":       "level0",
        "bnn-data-mono":      "level2",
    }.get(model_id, model_id.replace("bnn-", ""))
    p = os.path.join(eval_dir, f"test_predictions_{legacy_tag}.json")
    if not os.path.exists(p):
        old_dir = _OLD_SUR2 if model_id == REG_ID else _OLD_SUR0
        p = os.path.join(old_dir, f"test_predictions_{legacy_tag}.json")
    if not os.path.exists(p):
        p = os.path.join(_OLD, f"test_predictions_{legacy_tag}.json")
    if not os.path.exists(p):
        return None, None, None
    with open(p) as f:
        d = json.load(f)
    mu    = np.array(d.get("mu_te", d.get("mu")))
    sigma = np.array(d.get("sigma_te", d.get("sigma")))
    y     = np.array(d.get("y_te_true", d.get("y_true")))
    return mu, sigma, y
...
def _load_per_dim_metrics(model_id):
    ...
    Primary: BNN metrics_per_output_fixed.csv
    Legacy:  paper_metrics_per_dim_<tag>.csv
    """
    eval_dir = model_fixed_eval_dir(model_id)
    p_bnn = os.path.join(eval_dir, "metrics_per_output_fixed.csv")
    if os.path.exists(p_bnn):
        return pd.read_csv(p_bnn)

    legacy_tag = {
        "bnn-baseline":  "level0",
        "bnn-data-mono": "level2",
    }.get(model_id, model_id.replace("bnn-", ""))
    p = os.path.join(eval_dir, f"paper_metrics_per_dim_{legacy_tag}.csv")
    if not os.path.exists(p):
        p = os.path.join(_OLD, f"paper_metrics_per_dim_{legacy_tag}.csv")
    if not os.path.exists(p):
        return None
...
# ══════════════════════════════════════════════════════════════════════════
def fig1_framework():
    """Pipeline overview: inputs → HF solver → dataset → BNN surrogate → analyses."""
    for v, (fw, fh) in enumerate([(11, 4), (6, 8)], 1):
        ...
        if v == 1:
            ...
            box(5.6, 1.1, 2.2, 1.2, "BayesianMLP",            "constraint-regularized\nBNN surrogate", "#D6EAF8")
            ...
            arr(3.0, 2.6, 3.0, 2.0)
            box(1.5, 0.8, 3.0, 1.0, "BayesianMLP", "constraint-regularized\nBNN surrogate", "#D6EAF8")
...
# ══════════════════════════════════════════════════════════════════════════
def fig2_accuracy():
    mu_reg, sig_reg, y = _load_test_preds(REG_ID)
    mu_base, sig_base, _ = _load_test_preds(BASELINE_ID)
    ...
    m_reg  = _load_per_dim_metrics(REG_ID)
    m_base = _load_per_dim_metrics(BASELINE_ID)
    ...
    fig.suptitle("Figure 2 (v1) | BNN surrogate accuracy — constraint-regularized model",
                 fontweight="bold")
    ...
    ax.scatter(y_true, yp_reg, s=12, alpha=0.45, color=BLUE,
               edgecolors="none", label="Constraint-regularized BNN")
    ...
    r2_s = _r2(m_reg, "iteration2_max_global_stress")
    ...
    ax.barh(yp+h/2, r2_base, h, color=LGRAY, label="BNN baseline")
    ax.barh(yp-h/2, r2_reg,  h, color=BLUE,  label="Constraint-regularized BNN")
    ...
    if ood is not None:
        ...
        ax.bar(x-w/2, ol_base, w, color=LGRAY, label="BNN baseline")
        ax.bar(x+w/2, ol_reg,  w, color=BLUE,  label="Constraint-regularized BNN")
    ...
    fig.suptitle("Figure 2 (v2) | Predictive uncertainty — constraint-regularized BNN",
                 fontweight="bold")
    ...
    ax.set_xlim(lim); ax.set_ylim(lim)
    ax.set_xlabel("True stress (MPa)"); ax.set_ylabel("Predicted stress (MPa)")
    ax.set_title("(A)  Parity coloured by total predictive $\\sigma$", fontsize=9.5)
    cb = fig.colorbar(sc, ax=ax, shrink=0.8)
    cb.set_label("Predicted $\\sigma_\\mathrm{total}$ (MPa)", fontsize=8)
    clean_ax(ax)
...
# ══════════════════════════════════════════════════════════════════════════
def fig3_forward_uq():
    # Try BNN risk_propagation results first (per-model dir), fall back to legacy
    risk_dir = experiment_dir("risk_propagation")
    p_joint = os.path.join(risk_dir, REG_ID, "D1_nominal_joint_stress_keff.csv")
    ...
    # decoupled / iter1 reference statistics (synthesised)
...
# ══════════════════════════════════════════════════════════════════════════
def fig4_sobol():
    # Try BNN sensitivity results first
    sa_dir = experiment_dir("sensitivity")
    p_sobol = os.path.join(sa_dir, REG_ID, "sobol_results_with_ci.csv")
    ...
    def _filter_reg(sub_df, output):
        """Select rows for the regularized BNN model for a given output.
        Supports: model_id==REG_ID, or level==2 legacy encoding."""
        if "model_id" in sub_df.columns:
            return sub_df[(sub_df.output == output) & (sub_df.model_id == REG_ID)].copy()
        return sub_df[(sub_df.output == output) & (sub_df.level == 2)].copy()

    def _filter_base(sub_df, output):
        if "model_id" in sub_df.columns:
            return sub_df[(sub_df.output == output) & (sub_df.model_id == BASELINE_ID)].copy()
        return sub_df[(sub_df.output == output) & (sub_df.level == 0)].copy()

    def get_sobol(output, level):
        sub = _filter_reg(df, output)
        if len(sub) == 0:
            return None
    ...
    panel(axes[0], sl_reg,  sl_base,
          "(A)  Max global stress (coupled)\nconstraint-regularized BNN — baseline diamonds",
          "SS316_k_ref")
    ...
    for ax, (data, title) in zip(axes, [
            (sl_reg,  "Max global stress (constraint-regularized BNN)"),
            (kl_reg,  r"$k_\mathrm{eff}$ (constraint-regularized BNN)")]):
        if data is None:
            ax.axis("off"); continue
        out = data.output.values[0]
        st_sub = _filter_reg(df, out).set_index("input")
        for _, r in data.iterrows():
            s1 = r.S1
            sT = float(st_sub.loc[r["input"], "ST_mean"]) if r["input"] in st_sub.index else 0
...
# ══════════════════════════════════════════════════════════════════════════
def fig5_posterior():
    # BNN posterior summary (per-model) → legacy fallback
    post_dir_0404 = experiment_dir("posterior")
    p_alt = os.path.join(post_dir_0404, REG_ID, "posterior_summary_maintext.csv")
    post_path = p_alt if _exists(p_alt) else os.path.join(
        _OLD, "paper_posterior_hf_validation_summary_reduced_maintext.csv")

    p_ext_alt = os.path.join(post_dir_0404, REG_ID, "extreme_stress_risk_assessment.csv")
    ext_path  = p_ext_alt if _exists(p_ext_alt) else os.path.join(
        _OLD, "paper_extreme_stress_risk_assessment.csv")
    ...
    # ── v1: scatter + extreme-case bar chart ─────────────────────────────
    fig, axes = plt.subplots(1, 2, figsize=(12, 4.5))
    fig.suptitle("Figure 5 (v1) | Observation-driven posterior calibration (BNN)",
                 fontweight="bold")
...
def figA3_calibration_curve():
    ...
    fig.suptitle("Figure A3 | BNN surrogate calibration — stress output (constraint-regularized)",
                 fontweight="bold")
...
def figA4_hetero_sigma():
    """Total predictive σ vs true output — all 5 primary outputs."""
    mu_reg, sig_reg, y = _load_test_preds(REG_ID)
    if mu_reg is None:
    ...
    fig.suptitle("Figure A4 | Predictive $\\sigma_\\mathrm{total}$ vs true output (constraint-regularized BNN)",
                 fontweight="bold")
    ...
    for ax, out in zip(axes, PRIMARY):
        ...
        ax.set_ylabel("Predicted $\\sigma_\\mathrm{total}$", fontsize=8.5)
...
def figA5_per_output_metrics():
    """R² for all 15 outputs — BNN baseline vs constraint-regularized BNN."""
    m_reg  = _load_per_dim_metrics(REG_ID)
    m_base = _load_per_dim_metrics(BASELINE_ID)
    ...
    ax.barh(yp+h/2, r2_b, h, color=LGRAY, label="BNN baseline")
    ax.barh(yp-h/2, r2_r, h, color=BLUE,  label="Constraint-regularized BNN")
...
def figA7_bnn_schematic():
    """BayesianMLP architecture schematic (BNN version).

    Shows weight-space variational posteriors (mu_w, sigma_w), shared backbone,
    dual heads (mu / logvar), and MC sampling at inference.
    """
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.set_xlim(0, 10); ax.set_ylim(0, 5); ax.axis("off")
    fig.suptitle("Figure A7 | BayesianMLP architecture schematic (BNN)",
                 fontweight="bold", y=1.01)
    ...
    cols = [
        (1.0,  8,  "Input\n(8)"),
        (3.0,  5,  "Bayes-Hidden 1\n(width)"),
        (5.0,  5,  "Bayes-Hidden 2\n(width)"),
    ...
    ax.text(5.0, 4.8, r"$q(w)=\mathcal{N}(\mu_w, \mathrm{softplus}(\rho_w)^2)$  →  MC-sampled forward pass",
            ha="center", va="top", fontsize=9, color="#333", style="italic")
    ax.annotate("SiLU activation\n(no dropout — BNN self-regularizes)",
                xy=(3.0, 4.5), xytext=(1.8, 5.1),
...
def figA8_training_curves():
    ...
    models = [BASELINE_ID, REG_ID]
    model_colors = {BASELINE_ID: GRAY, REG_ID: BLUE}

    fig, axes = plt.subplots(1, 2, figsize=(11, 4))
    fig.suptitle("Figure A8 | BNN training loss curves", fontweight="bold")
    ...
    if not has_any:
        for ax in axes:
            ...
            ax.axis("off")
        ax0 = axes[0]
        ax0.set_title("Figure A8 | BNN training curves (placeholder)",
