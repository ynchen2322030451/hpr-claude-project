...
>
> **Last data update**: 2026-04-25 (added held-out posterior validation, cross-seed Sobol, MC noise bound, sampling mismatch analysis)
...
### 13a. Heldout case row indices

Row indices (test-split-relative): [291, 96, 25, 45, 269, 498, 458, 170, 274, 273, 206, 191, 101, 454, 74, 76, 223, 57]

| Category | Cases | Stress range (MPa) |
|----------|-------|---------------------|
| low (6) | 291, 96, 25, 45, 269, 498 | 107.5 -- 120.1 |
| near (6) | 458, 170, 274, 273, 206, 191 | 121.5 -- 130.7 |
| high (6) | 101, 454, 74, 76, 223, 57 | 156.4 -- 209.9 |

### 13b. Overall results (bnn-phy-mono, 4-chain)
...
- Low-stress coverage (0.792) is the weakest category, consistent with canonical (0.667).
  The discrepancy in absolute level between canonical low (0.667) and heldout low (0.792)
  is within expected sampling variation for n=6 cases per category.
- Near-stress coverage (0.972) remains the strongest category.
- Coverage is stable across noise levels (0.833--0.875), with 1% noise slightly lower.
...
**Method**: 5 independent Saltelli sampling seeds [2026, 2027, 2028, 2029, 2030], same canonical bnn-phy-mono model
**Settings**: N_base = 4096, R = 50 replicates per seed, M = 30 weight samples per evaluation

### 14a. Stress (iteration2_max_global_stress) — cross-seed stability

| Input | S1 (mean +/- std) | Rank (mean) | Rank range | Rank stable |
|-------|-------------------|-------------|------------|-------------|
| E_intercept | **0.546 +/- 0.002** | 1.0 | [1, 1] | True |
| alpha_base | **0.199 +/- 0.007** | 2.0 | [2, 2] | True |
| SS316_k_ref | 0.060 +/- 0.005 | 3.6 | [3, 4] | True |
| nu | 0.060 +/- 0.002 | 3.4 | [3, 4] | True |
| alpha_slope | 0.041 +/- 0.005 | 5.2 | [5, 6] | True |
| E_slope | 0.035 +/- 0.004 | 5.8 | [5, 6] | True |
| SS316_T_ref | 0.019 +/- 0.003 | 7.0 | [7, 7] | True |
| SS316_alpha | 0.001 +/- 0.003 | 8.0 | [8, 8] | True |

**All 8 stress parameters are rank-stable across seeds.**

### 14b. keff (iteration2_keff) — cross-seed stability

| Input | S1 (mean +/- std) | Rank (mean) | Rank range | Rank stable |
|-------|-------------------|-------------|------------|-------------|
| alpha_base | **0.769 +/- 0.002** | 1.0 | [1, 1] | True |
| alpha_slope | **0.194 +/- 0.007** | 2.0 | [2, 2] | True |
| nu | 0.018 +/- 0.004 | 3.0 | [3, 3] | True |
| E_intercept | 0.007 +/- 0.005 | 4.6 | [4, 5] | True |
| SS316_T_ref | 0.007 +/- 0.004 | 5.8 | [4, 7] | **False** |
| SS316_alpha | 0.004 +/- 0.007 | 6.2 | [4, 8] | **False** |
| SS316_k_ref | 0.003 +/- 0.006 | 6.2 | [4, 8] | **False** |
| E_slope | 0.001 +/- 0.003 | 7.2 | [6, 8] | **False** |

**Top 3 keff ranks stable. Ranks 4--8 unstable (all S1 < 0.01 — negligible sensitivity).**

### 14c. Comparison with canonical single-seed Sobol (Section 5)

| Output.Input | Canonical S1 (seed 2026) | Cross-seed S1 (mean +/- std) | Consistent |
|--------------|--------------------------|-------------------------------|------------|
| stress.E_intercept | 0.579 | 0.546 +/- 0.002 | Rank 1 preserved; magnitude shift ~0.03 |
| stress.alpha_base | 0.169 | 0.199 +/- 0.007 | Rank 2 preserved; magnitude shift ~0.03 |
| keff.alpha_base | 0.785 | 0.769 +/- 0.002 | Rank 1 preserved; magnitude shift ~0.02 |
| keff.alpha_slope | 0.179 | 0.194 +/- 0.007 | Rank 2 preserved; magnitude shift ~0.02 |

**Note**: Cross-seed S1 values differ from canonical single-seed (Section 5a/5b) because
the canonical analysis uses a single Saltelli seed (2026) while cross-seed averages over
5 seeds with independent Saltelli samples. The rank ordering is consistent; magnitude
differences (~0.02--0.03) reflect Saltelli sampling variability, not model instability.

---

## 15. Sobol MC Noise Analytical Bound

**Source**: derived analytical result, not from experiment files
**Method**: BNN weight-sampling MC introduces variance in model predictions;
this bounds the resulting bias on first-order Sobol indices.

### 15a. Bound formula

BNN MC bias on S1: |Delta S1| <= (1 - R^2) / M

where R^2 is the model's test-set coefficient of determination and M is the
number of MC weight samples per forward evaluation.

### 15b. Numerical bounds (M = 30)

**Conservative bound (using single-draw test-set R^2 from Section 2a)**:

| Output | Test R^2 | 1 - R^2 | |Delta S1| upper bound |
|--------|----------|---------|-----------------------|
| stress | 0.9438 | 0.0562 | 0.0562 / 30 = **1.9 x 10^-3** |
| keff | 0.8492 | 0.1508 | 0.1508 / 30 = **5.0 x 10^-3** |

**Tighter bound (using ensemble-mean R^2, if applicable)**:

| Output | R^2 (ensemble mean) | |Delta S1| upper bound |
|--------|---------------------|-----------------------|
| stress | ~0.997 | ~1.0 x 10^-4 |
| keff | ~0.999 | ~3.3 x 10^-5 |

The tighter bound applies if R^2 in the formula refers to the variance explained
by the MC ensemble mean (averaging over M weight draws), which is higher than
single-draw R^2. In either case, the MC noise bias is negligible compared to
dominant S1 indices (stress E_intercept 0.546--0.579, keff alpha_base 0.769--0.785)
and bootstrap CI widths (~0.01--0.02).

---

## 16. Training Distribution vs Sobol Sampling Mismatch

**Source**: analysis of training data distribution and Sobol sampling range
**Status**: analytical assessment, not a standalone experiment

### 16a. Key findings

- Training data uses **truncated normal** distributions for input parameters.
  The outer 50% of each parameter's range contains only **9--18%** of training points.
- Sobol analysis uses **uniform** sampling over the same parameter range.
  No extrapolation beyond training bounds occurs.
- The distribution mismatch means the model is less densely trained in the
  outer regions where Sobol sampling places equal weight.
- **Cross-seed Sobol validation (Section 14) confirms this mismatch is not material**:
  rank ordering is perfectly preserved for all dominant parameters, and S1
  cross-seed standard deviations are small (0.002--0.007).

### 16b. Manuscript implication

This mismatch should be acknowledged as a limitation but does not invalidate
the Sobol results. The recommended wording: "The surrogate was trained on
truncated-normal-distributed samples; Sobol sensitivity indices are computed
under a uniform-sampling assumption over the same support. Cross-seed
validation (5 independent Saltelli seeds) confirms rank stability."
