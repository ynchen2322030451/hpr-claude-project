...
%CN: 约2900次OpenMC–FEniCS耦合仿真数据训练。含物理规律单调性约束的正则化变体（单调正则化代理）通过嵌入
%CN: 物理一致单调性约束，在测试集上达到负对数似然0.305，应力R²=0.929（均方根误差7.9 MPa）。
%CN: 前向传播20000个先验蒙特卡洛样本揭示：耦合稳态应力均值较解耦预测低39 MPa（20%），
%CN: 标准差降低47%；keff不确定性压缩183倍；超过131 MPa设计阈值的概率为0.847。
%CN: Sobol分析识别E_intercept为应力主控因素（S₁=0.598，CI 0.596–0.601），alpha_base为keff主控因素
%CN: （S₁=0.775，CI 0.774–0.776），表明两条独立物理传播路径。后验标定对全部10个极端应力案例（真实应力
...
as the dominant stress driver ($S_1 = 0.598$, 90\%~CI:~0.596--0.601) and thermal expansion
coefficient~($\alpha_{\mathrm{base}}$) as the dominant $k_{\mathrm{eff}}$ driver ($S_1 = 0.775$,
CI:~0.774--0.776), indicating distinct dominant physical pathways with limited cross-interaction.
Observation-driven posterior calibration demonstrates that, for all ten extreme-stress
scenarios with observed stress $\geq\SI{220}{MPa}$, the posterior exceedance probability
converges to~1.0 (prior: 0.84--0.96), effectively concentrating posterior mass in the
high-risk parameter region and enabling reliable in-situ risk stratification.
...
OOD robustness for the stress response under $\alpha_{\mathrm{base}}$ extrapolation.
...
$k_{\mathrm{ref,SS316}}$& SS316 thermal conductivity (at $T_{\mathrm{ref}}$) & W/(m$\cdot$K) & Uniform \\
$k_{\mathrm{slope,SS316}}$& SS316 thermal cond.\ temp.\ coefficient$^\dagger$ & W/(m$\cdot$K$^2$) & Uniform \\
\botrule
\end{tabular*}
\footnotetext{$^\dagger$~$k_{\mathrm{slope,SS316}}$ is the temperature slope of SS316 thermal
  conductivity, distinct from the structural thermal expansion coefficients
  $\alpha_{\mathrm{base}}$ and $\alpha_{\mathrm{slope}}$.
  Prior bounds: \textbf{[TBD]}.}
\end{table}
...
For 20~benchmark cases drawn from the calibration pool, MCMC~\cite{ref:mcmc}
...
These are the structural and thermal expansion properties most directly constrained
by the available single-point stress observations.
The remaining four parameters ($E_{\mathrm{slope}}$, $T_{\mathrm{ref,SS316}}$,
$k_{\mathrm{ref,SS316}}$, $k_{\mathrm{slope,SS316}}$) are held at their true
test-case input values, reflecting a partial-observation scenario in which the
SS316 thermal properties are treated as known from prior material characterisation.
Although $k_{\mathrm{ref,SS316}}$ is the dominant stress driver (Section~\ref{sec:results_sobol}),
incorporating it as a calibration parameter would require independent thermal
conductivity measurements; the present protocol represents a lower bound on
achievable posterior uncertainty.
This design choice is an acknowledged limitation discussed in Section~\ref{sec:discussion}.
Observations consist of the true outputs corrupted by Gaussian noise.
Each MCMC run generates 1200~posterior samples (acceptance rates 0.47--0.61).
Note that posterior predictions are validated against the existing HF dataset via
nearest-neighbour lookup (not a true HF rerun at the posterior mean); this constitutes
a proxy validation whose limitations are discussed in Section~\ref{sec:discussion}.
...
%CN: 物理正则化代理（单调正则化代理）测试NLL 0.305，优于基线（纯数据驱动基线代理）的0.359（降低15%）。
%CN: 5个主要输出量均值R²=0.795（单调正则化代理）vs. 0.790（纯数据驱动基线代理）。逐输出分析：iter2应力
%CN: 两模型均达到R²=0.929，RMSE=7.9 MPa，PICP₉₀分别为0.913和0.931。keff：单调正则化代理
%CN: R²=0.874 vs. 纯数据驱动基线代理 R²=0.865。温度输出R²约0.58–0.59（两模型），壁面膨胀R²=0.996。
%CN: OOD关键结果：alpha_base外推，纯数据驱动基线代理应力R²=0.774，单调正则化代理为0.944（提升+0.170）。

The physics-regularized surrogate achieves test NLL~$= 0.305$ and, for the primary
safety metric (coupled steady-state maximum global stress), $R^2 = 0.929$,
RMSE~$=\SI{7.9}{MPa}$, and PICP$_{90} = 0.913$; mean $R^2$ across the five primary
outputs is 0.795 (Table~\ref{tab:accuracy}).
This model is selected as the primary surrogate for all subsequent analyses
on the basis of its distributional calibration and robustness under distributional
shift, demonstrated below.
...
The physics-regularized model retains coupled steady-state stress $R^2 = 0.944$
under this extrapolation, compared to 0.774 for the baseline---a difference of $\Delta R^2 = +0.170$ directly attributable to the enforced
$\alpha_{\mathrm{base}}$--stress monotonicity constraint.
This improvement is specific to the stress response under $\alpha_{\mathrm{base}}$
extrapolation; OOD performance on other outputs and input features is heterogeneous
(see Appendix~\ref{sec:appendix_ood} for the full comparison).

\begin{table}[ht]
\caption{%
  Per-output accuracy of the physics-regularized surrogate (Regularized) on the
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    ($R^2=0.929$, RMSE~$=\SI{7.9}{MPa}$);
    (B)~$R^2$ for all five primary outputs comparing the baseline and
    physics-regularized surrogates;
    (C)~OOD robustness for $\alpha_{\mathrm{base}}$ extrapolation:
    physics regularization provides $\Delta R^2=+0.170$ for the stress response.
  }\label{fig:accuracy}
\end{figure}
...
\caption{%
  ...
  Regularized shows the largest improvement over Baseline for $\alpha_{\mathrm{base}}$
  extrapolation.
}\label{tab:ood}
\begin{tabular}{@{}lrrr@{}}
\toprule
OOD feature & $n_{\mathrm{OOD}}$ & Baseline $R^2$ & Regularized $R^2$ \\
\midrule
...
This feedback is absent in the decoupled prediction; omitting it inflates stress
standard deviation by approximately $2\times$.

The probability of exceeding the design threshold $\tau = \SI{131}{MPa}$ is
estimated at $P_{\mathrm{exceed}} = 0.847$ under the prior (epistemic, mean predictions).
...
This exceedance probability matches prior high-fidelity analysis of the nominal
MegaPower design, which reports peak monolith stresses reaching \SI{169}{MPa} under
normal operating conditions~\cite{ref:zhang2025energy};
the training dataset predominantly samples the above-threshold operating regime,
and $P_{\mathrm{exceed}} = 0.847$ reflects the position of the nominal design
relative to the material yield limit, not a~surrogate artefact.
...
\begin{table}[ht]
\caption{%
  Comparison of decoupled single-pass and coupled steady-state uncertainty distributions
  for key outputs, from \num{20000} prior Monte~Carlo samples through the
  monotone-regularized surrogate (epistemic, mean predictions).
}\label{tab:forward_uq}
\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lrrrrrr@{}}
\toprule
Output & \multicolumn{3}{c}{Decoupled single-pass} & \multicolumn{3}{c}{Coupled steady-state} \\
...
\midrule
\multicolumn{7}{l}{$P_{\mathrm{exceed}}(\tau = \SI{131}{MPa}) = 0.847$
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    distributions (20\,000 prior Monte~Carlo samples, Regularized surrogate,
    ...
    (C)~$k_{\mathrm{eff}}$ coupled distribution ($183\times$ zoomed); the coupling
    collapses $k_{\mathrm{eff}}$ uncertainty from $\sim\!6250$~pcm to $\sim\!34$~pcm.
  }\label{fig:forward_uq}
\end{figure}

\subsection{Global sensitivity analysis}\label{sec:results_sobol}

%CN: Sobol结论（0404 data-mono 单调正则化代理 canonical）：两个主要输出受独立物理路径控制。
%CN: iter2应力：E_intercept主控（S₁=0.598，CI 0.596–0.601），alpha_base次之
%CN: （S₁=0.148，CI 0.144–0.151），SS316_k_ref第三（S₁=0.080，CI 0.077–0.083）；
%CN: 其余参数CI均不稳定（包含零或边界接近零）。
%CN: iter2 keff：alpha_base主控（S₁=0.775，CI 0.774–0.776），alpha_slope次之
%CN: （S₁=0.177，CI 0.174–0.180）；其余6个参数CI包含零，不作为稳定主控因素。
...
For coupled steady-state maximum global stress (Regularized surrogate), the Young's modulus
intercept ($E_{\mathrm{intercept}}$) is the dominant first-order factor with
$S_1 = 0.598$ (90\%~CI:~0.596--0.601), accounting for approximately 60\% of the
first-order variance decomposition.
The thermal expansion coefficient ($\alpha_{\mathrm{base}}$) ranks second with
$S_1 = 0.148$ (CI:~0.144--0.151), and the SS316 reference thermal conductivity
($k_{\mathrm{ref,SS316}}$) is third with $S_1 = 0.080$ (CI:~0.077--0.083).
...
The smaller but non-zero $S_1$ of $k_{\mathrm{ref,SS316}}$ ($= 0.080$) reflects a~
real thermal-conduction pathway that is partially masked by the stronger elastic effect.

For coupled steady-state $k_{\mathrm{eff}}$ (Regularized), $\alpha_{\mathrm{base}}$ is the single
dominant factor with $S_1 = 0.775$ (CI:~0.774--0.776), followed by
$\alpha_{\mathrm{slope}}$ ($S_1 = 0.177$, CI:~0.174--0.180).
...
\caption{%
  ...
  maximum global stress and $k_{\mathrm{eff}}$ (Regularized surrogate, data-mono).
...
\midrule
Max.\ global stress & $E_{\mathrm{intercept}}$    & 0.598 & 0.596 & 0.601 \\
                    & $\alpha_{\mathrm{base}}$    & 0.148 & 0.144 & 0.151 \\
                    & $k_{\mathrm{ref,SS316}}$   & 0.080 & 0.077 & 0.083 \\
\midrule
$k_{\mathrm{eff}}$ & $\alpha_{\mathrm{base}}$    & 0.775 & 0.774 & 0.776 \\
                   & $\alpha_{\mathrm{slope}}$   & 0.177 & 0.174 & 0.180 \\
\botrule
\end{tabular}
\footnotetext{%
  Source: \texttt{experiments\_0404/sensitivity/data-mono/sobol\_results.csv}.
  Base sample $N_S = \num{4096}$;
...
For each case, the Metropolis-Hastings sampler was run for 8000~total iterations with
2000~burn-in steps discarded and a~thinning factor of~5, yielding 1200~effective
posterior samples per case.
Acceptance rates across all benchmark cases ranged from 0.47 to 0.61, consistent with
adequate single-chain Metropolis-Hastings exploration for the 4-parameter posterior.
Convergence was assessed by inspecting trace plots: after the burn-in period, all four
calibrated parameters exhibit stationary random fluctuations within consistent ranges
without directional drift, confirming adequate posterior exploration.
Note that posterior predictions are validated against the nearest-neighbour entry in
the HF dataset rather than a true HF rerun at the posterior mean; this proxy
validation is used throughout this section.
...
For high-stress cases, $E_{\mathrm{intercept}}$ is skewed toward lower values---reduced
stiffness provides stress relief---while $\alpha_{\mathrm{base}}$ concentrates at the
lower end, as lower thermal expansion suppresses the coupling-driven stress amplification.
The joint posterior of $E_{\mathrm{intercept}}$ and $\alpha_{\mathrm{base}}$ shows a~
pronounced compensation effect: feasible solutions cluster where both parameters are
simultaneously small, matching the multiplicative stress dependence
$\sigma \propto E \cdot \alpha \cdot T$.
...
these cases (mean absolute error $<\SI{6}{MPa}$), confirming that the calibration
localises the posterior in the correct high-risk parameter region.
Sobol analysis (Section~\ref{sec:results_sobol}) identifies low $k_{\mathrm{ref,SS316}}$
as the dominant stress driver; following extreme-stress observations, the posterior
systematically shifts $E_{\mathrm{intercept}}$ and $\alpha_{\mathrm{base}}$ into
the high-stress parameter region.
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    (A)~Posterior mean stress prediction vs.\ observed stress for 20~benchmark
    test cases (nearest-neighbour HF proxy validation); posterior predictions
    closely track the diagonal, improving on the prior mean (grey crosses).
    (B)~Prior and posterior exceedance probabilities
    $P_{\mathrm{exceed}}(\SI{131}{MPa})$ for the 10~extreme-stress cases
...
regions.
Physics regularization is preferred on three grounds.
First, in-distribution accuracy is comparable to the baseline ($\Delta R^2 \approx +0.005$;
Appendix~\ref{sec:appendix_baseline}).
Second, under $\alpha_{\mathrm{base}}$ extrapolation the regularized model retains
$R^2 = 0.944$ while the baseline falls to 0.774 ($\Delta R^2 = +0.170$);
the unconstrained baseline violates the physical monotonicity in low-density input
regions, and the Spearman-rank regularizer corrects this directly.
Third, the Sobol first-order index for $E_{\mathrm{intercept}}$ shifts from
$S_1 = 0.613$ (baseline, Baseline) to $S_1 = 0.598$ (regularized, Regularized),
reflecting that the physics constraint redistributes attribution from the dominant elastic
factor toward physically consistent secondary factors ($\alpha_{\mathrm{base}}$, $k_{\mathrm{ref,SS316}}$).
...
$E_{\mathrm{intercept}}$ accounts for approximately 60\% of coupled stress variance
($S_1 = 0.598$); tighter manufacturing control of the Young's modulus intercept
is therefore warranted, alongside---rather than instead of---the conventional focus
on thermal conductivity.
The secondary but non-zero contribution of $k_{\mathrm{ref,SS316}}$ ($S_1 = 0.080$)
indicates that thermal-conductivity uncertainty should not be entirely neglected in
structural safety margins, even though elastic stiffness is the primary driver.
Conversely, $\alpha_{\mathrm{base}}$ is the dominant reactivity driver ($S_1 = 0.775$)
but contributes only moderately to stress ($S_1 = 0.148$); the two physical pathways
can therefore be targeted by partially decoupled measurement and control strategies,
reducing cross-interference in the overall safety-analysis workflow.
...
The OOD tests show that Regularized degrades more gracefully than Baseline in
extrapolation, but absolute accuracy still decreases outside the training range.
The MCMC calibration fixes four of the eight input parameters at their true
test-case values (SS316 thermal properties), treating them as known from material
characterisation; in practice these values would not be precisely known, so the
reported posterior uncertainty is a lower bound.
A~full 8-dimensional calibration, or one that treats $k_{\mathrm{ref,SS316}}$ as
unknown, would increase posterior uncertainty but require independent thermal
...
\item \textbf{Physics regularization improves generalization.}
  The monotone-regularized HeteroMLP, trained with physics-consistent monotonicity
  constraints, matches in-distribution accuracy (test NLL 0.305, stress $R^2 = 0.929$)
  while substantially improving OOD performance for $\alpha_{\mathrm{base}}$ extrapolation
  ($R^2$: 0.774 $\to$ 0.944).
...
\item \textbf{Distinct sensitivity pathways exist.}
  Sobol analysis identifies $E_{\mathrm{intercept}}$ as the dominant stress driver
  ($S_1 = 0.598$) and $\alpha_{\mathrm{base}}$ as the dominant $k_{\mathrm{eff}}$ driver
  ($S_1 = 0.775$), indicating that design interventions for stress reduction and
...
%CN: 极值外推测试（每次取分布两端的约20%样本作为OOD集），对比纯数据驱动基线代理与单调正则化代理在
...
$k_{\mathrm{eff}}$ under each OOD feature, for both Baseline and Regularized.

The largest Regularized improvement is for $\alpha_{\mathrm{base}}$ extrapolation
...
For $\nu$, Regularized shows a~slight stress $R^2$ decrease relative to Baseline
...
\caption{%
  ...
  $R^2$ for Baseline and Regularized surrogates, by OOD input feature.
  $\Delta R^2 = $ Regularized $-$ Baseline (stress).
}\label{tab:ood_full}
...
& & Baseline & Regularized & Baseline & Regularized \\
\midrule
...
\footnotetext{Source: \texttt{paper\_ood\_multi\_feature\_summary.csv}.
  ...
  embedded in the Regularized objective.}
\end{table}
...
%CN: 附录Baseline：纯数据驱动基线代理与单调正则化代理在测试集上5个主要输出量的完整精度对比。
%CN: 两模型R²差距极小（应力R²相同=0.929），主要差距体现在NLL（0.359→0.305，改善15%）
%CN: 和keff PICP90（0.956→0.975），以及OOD泛化（见附录OOD）。
Table~\ref{tab:baseline_compare} compares Baseline and Regularized on the held-out
...
($0.359$ for Baseline vs.\ $0.305$ for Regularized, a~15\% reduction) and
...
The primary motivation for selecting Regularized as the main model is therefore its
...
\begin{table}[ht]
\caption{%
  Comparison of Baseline (baseline) and Regularized (physics-regularized) on the
  held-out test set ($n = 435$), primary outputs only.
  Test NLL: Baseline = 0.359, Regularized = 0.305.
}\label{tab:baseline_compare}
\begin{tabular*}{\textwidth}{@{\extracolsep\fill}llrrrr@{}}
\toprule
Output & & \multicolumn{2}{c}{$R^2$} & \multicolumn{2}{c}{PICP$_{90}$} \\
\cmidrule(lr){3-4}\cmidrule(lr){5-6}
       & Pass & Baseline & Regularized & Baseline & Regularized \\
\midrule
...
\footnotetext{%
  ...
  RMSE for max.\ global stress: Baseline = \SI{7.93}{MPa}, Regularized = \SI{7.91}{MPa}.}
\end{table}
...
probability at all three design thresholds for both Baseline and Regularized.
...
\caption{%
  ...
  $k_{\mathrm{eff}}$, Baseline and Regularized.
...
\midrule
\multirow{8}{*}{Stress}
  & $E_{\mathrm{slope}}$       & 0.053 & 0.045 & 0.041 & 0.048 \\
  & $E_{\mathrm{intercept}}$   & 0.613 & 0.598 & 0.596 & 0.601 \\
  & $\nu$                      & 0.051 & 0.051 & 0.047 & 0.054 \\
  & $\alpha_{\mathrm{base}}$   & 0.157 & 0.148 & 0.144 & 0.151 \\
  & $\alpha_{\mathrm{slope}}$  & 0.024 & 0.026 & 0.023 & 0.029 \\
  & $T_{\mathrm{ref,SS316}}$   & 0.024 & 0.021 & 0.018 & 0.025 \\
  & $k_{\mathrm{ref,SS316}}$   & 0.065 & 0.080 & 0.077 & 0.083 \\
  & $\alpha_{\mathrm{SS316}}$  & 0.001$^\dag$ & 0.002$^\dag$ & $-$0.002 & 0.005 \\
\midrule
\multirow{8}{*}{$k_{\mathrm{eff}}$}
  & $E_{\mathrm{slope}}$       & 0.003$^\dag$ & 0.004$^\dag$ & 0.000 & 0.007 \\
  & $E_{\mathrm{intercept}}$   & 0.004 & 0.003$^\dag$ & 0.000 & 0.006 \\
  & $\nu$                      & 0.016 & 0.033 & 0.029 & 0.036 \\
  & $\alpha_{\mathrm{base}}$   & 0.809 & 0.775 & 0.774 & 0.776 \\
  & $\alpha_{\mathrm{slope}}$  & 0.162 & 0.177 & 0.174 & 0.180 \\
  & $T_{\mathrm{ref,SS316}}$   & 0.006 & 0.004$^\dag$ & 0.000 & 0.007 \\
  & $k_{\mathrm{ref,SS316}}$   & 0.009 & 0.007 & 0.004 & 0.010 \\
  & $\alpha_{\mathrm{SS316}}$  & 0.006 & 0.005 & 0.002 & 0.008 \\
\botrule
\end{tabular*}
\footnotetext{Source: \texttt{experiments\_0404/sensitivity/\{baseline,data-mono\}/sobol\_results.csv}.
  L0~=~baseline surrogate; L2~=~data-mono (physics-regularized) surrogate.}
\end{table}

\section{Per-output accuracy table (all 15 outputs)}\label{sec:appendix_metrics}

%CN: 附录S4：全部15个输出量的完整精度表（单调正则化代理）。
The full per-output accuracy metrics for the Regularized surrogate on the test set
are provided in Table~\ref{tab:accuracy_full}.

\begin{table}[ht]
\caption{%
  Complete per-output accuracy metrics, Regularized surrogate, test set ($n=435$).
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    stress output (Regularized, test set $n=435$).
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    global stress (Regularized, test set).
...
\begin{figure}[ht]
  ...
  \caption{%
    Per-output accuracy for all 15 physics outputs (Regularized surrogate, test set).
